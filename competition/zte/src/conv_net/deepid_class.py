...
class DeepID:
    ...
    def load_data_deepid(self, dataset_file, batch_size):
        ...
        self.n_train_batches = self.train_set_x.get_value(borrow=True).shape[
                                   0] / batch_size
        self.n_valid_batches = self.valid_set_x.get_value(borrow=True).shape[
                                   0] / batch_size
    ...
    def build_layer_architecture(self, n_hidden, n_out, acti_func=relu):
        ...
        deepid_input = T.concatenate(
            [layer3_output_flatten, layer4_output_flatten], axis=1)

        self.deepid_layer = HiddenLayer(self.rng,
                                        input=deepid_input,
                                        n_in=numpy.prod(self.result_image_shape[
                                                        1:]) + numpy.prod(self.layer4_image_shape[1:]),
    ...
    def build_test_valid_model(self):
        self.test_valid_model = theano.function(
            inputs=[
                self.index],
            outputs=self.softmax_layer.errors(
                self.y),
            givens={
    ...
    def build_test_train_model(self):
        self.test_train_model = theano.function(
            inputs=[
                self.index],
            outputs=self.softmax_layer.errors(
                self.y),
        ...
        while epoch < n_epochs:
            ...
            for minibatch_index in xrange(self.n_train_batches):
                minibatch_cost = self.train_model(
                    minibatch_index, learning_rate)
                train_loss = self.test_train_model(minibatch_index)
                train_losses.append(train_loss)

                line = '\r\tepoch %i, minibatch_index %i/%i, error %f' % (
                    epoch, minibatch_index, self.n_train_batches, train_loss)
                sys.stdout.write(line)
                sys.stdout.flush()

            valid_losses = [
                self.test_valid_model(i) for i in xrange(
...
if __name__ == '__main__':
    params_file = 'E:/soft/Project/zte/ztedata/youtube_and_img/params_file_youtube'
    train_folder = 'E:/soft/Project/zte/ztedata/CBSR_webface/train_rgb_set_file_vector/'
    valid_folder = 'E:/soft/Project/zte/ztedata/CBSR_webface/test_rgb_set_file_vector/'
    # valid_folder, train_folder = dataset
    simple_deepid(learning_rate=0.01, n_epochs=60, dataset=(valid_folder, train_folder), params_file=params_file,
                  nkerns=[20, 40, 60, 80], batch_size=500, n_hidden=160, n_out=1953, acti_func=relu)
