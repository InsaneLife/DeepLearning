...
def get_train_data(folder):
    ...
    for key in train_map:
        ...
        for i in range(1, len(train_map[key])):
            vector_2 = np.hstack((train_map[key][i - 1], train_map[key][i]))
        ...
        for i in range(0, len(train_map[key])):
            if last_key != 0:
                vector_2 = np.hstack((train_map[last_key][i], train_map[key][i]))
                train_vector.append(vector_2)
...
def get_zte_test(zte_datalist, zte_rgb, test_folder):
    test_vector = []
    test_label = []
    ...
    for each in datalist:
        ...
        if rgb_map.has_key(img1_name) and rgb_map.has_key(img2_name):
            test_vector.append(np.hstack((pkl_map[int(rgb_map[img1_name])], pkl_map[int(rgb_map[img2_name])])))
