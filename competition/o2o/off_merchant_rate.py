...
def insert_mysql(t):
    conn = MySQLdb.connect(host='192.168.2.249', user='root', passwd='admin', db='o2o')
    cursor = conn.cursor()
    sql1 = "insert into off_um_rate values(%s,%s,%s,%s,%s)"
    for each in t:
...
def main():
    # out = open("E:\\Datamining\\tianchi\\o2o\\off_m_rate.txt", "w")
    sql = "SELECT merchant_id as merchant_id,tag,COUNT(*) AS num FROM off_t1 o GROUP BY o.merchant_id,o.tag order by merchant_id"
    rows = mysqlQuery(sql)
    maps, result = {}, []
    for row in rows:
        if maps.has_key(row["merchant_id"]):
            maps[row["merchant_id"]][row["tag"]] += row["num"]
        else:
            maps[row["merchant_id"]] = [0, 0, 0]
            maps[row["merchant_id"]][row["tag"]] += row["num"]
    ...
    for ma in maps:
        ...
        # out.close()
    print "Insert Mysql..."
    excuteSql("TRUNCATE TABLE off_m_rate")
    insert_mysql(result)
