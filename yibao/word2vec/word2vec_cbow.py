...
# https://github.com/ahangchen/GDLnotes/blob/master/src/rnn/cbow.py
filename = '..//../../data/yibao/panzhihua/oneline_cut.txt'
...
def read_data(filename):
    ...
    with open(filename) as f:
        data = tf.compat.as_str(f.readline().decode('utf-8')).split()
    return data
...
def write_with_labels(low_dim_embs, labels,
                      out_file='..//..//../data/yibao/panzhihua/word2vec/kc25k2_word2vec_cbow.txt'):
    out = open(out_file, 'w')
...
def write_to_scatter(low_dim_embs, labels,
                     out_file='../../../data/yibao/panzhihua/word2vec//kc25k2_word2vec_cbow_scatter.txt'):
    out = open(out_file, 'w')
    for i, label in enumerate(labels):
        x, y = low_dim_embs[i, :]
...
write_to_scatter(two_d_embeddings, labels)
